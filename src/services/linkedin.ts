...
export async function publishLinkedInPost(
  ...
  customToken?: string,
  customUrn?: string,
  mediaFiles?: Array<{ name: string; type: string; storagePath?: string; readUrl?: string; base64?: string; }>
    ...
    if (mediaFiles && mediaFiles.length > 0) {
      for (const mediaFile of mediaFiles) {
        ...
        try {
          ...
          return { error: `LinkedIn registerUpload failed for "${mediaFile.name}": ${axiosError.response?.status} - ${errText}` };
        }
        ...
        if (!uploadUrl || !mediaUrn) {
          return { error: `LinkedIn registerUpload response was missing upload URL or asset URN for "${mediaFile.name}"` };
        }

        // 3. Obtain media body (stream from readUrl or decode base64 buffer)
        let bodyData: unknown;
        if (mediaFile.readUrl) {
          console.log(`[LinkedIn-Publish] Downloading media from: ${mediaFile.readUrl} for "${mediaFile.name}"`);
          const downloadRes = await axios.get(mediaFile.readUrl, { responseType: "stream" });
          bodyData = downloadRes.data;
        } else if (mediaFile.base64) {
          console.log(`[LinkedIn-Publish] Decoding base64 media data (local fallback) for "${mediaFile.name}".`);
          const base64Data = mediaFile.base64.split(",")[1] || mediaFile.base64;
          bodyData = Buffer.from(base64Data, "base64");
        } else {
          return { error: `mediaFile "${mediaFile.name}" is missing both readUrl and base64 data` };
        }
        ...
        try {
          ...
          const axiosError = err as { response?: { status?: number; data?: unknown }; message?: string };
          const errText = axiosError.response?.data ? JSON.stringify(axiosError.response.data) : axiosError.message;
          return { error: `LinkedIn file binary upload failed for "${mediaFile.name}": ${axiosError.response?.status} - ${errText}` };
        }
    ...
    const payload = {
      ...
      specificContent: {
        "com.linkedin.ugc.ShareContent": {
          ...
          shareMediaCategory,
          media: mediaUrns.length > 0 ? mediaUrns.map((urn, idx) => {
